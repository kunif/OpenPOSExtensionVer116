...
@ECHO Unregistration of OpenPOS.Extension file
CHDIR "build"
FOR %%F IN (OpenPOS*.dll) DO "%SystemRoot%\Microsoft.NET\Framework\v4.0.30319\RegAsm.exe" %%F /unregister /silent
...
@ECHO Unregistration of OpenPOS.Extension file
CHDIR "build"
FOR %%F IN (OpenPOS*.dll) DO "%SystemRoot%\Microsoft.NET\Framework64\v4.0.30319\RegAsm.exe" %%F /unregister /silent
FOR %%F IN (OpenPOS*.dll) DO "%SystemRoot%\Microsoft.NET\Framework\v4.0.30319\RegAsm.exe" %%F /unregister /silent
FOR %%F IN (OpenPOS*.dll) DO .\gacutil /u %%~nF /silent
